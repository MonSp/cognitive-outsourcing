...
**Important distinction**: This Kitchen-benchmark quality drop reflects *task-level information coverage* in a complex multi-step scenario (35 tool calls with interleaved reasoning), not KV-cache factual recall degradation per se. Separate deep validation experiments (P2, Section 7.2) demonstrate that pure factual recall from KV-cache shows **no observable degradation across 64 injection rounds and 13.6K cache tokens** on Qwen3.5-0.8B, with Gemma-4-E2B achieving perfect 1.00 recall at 32 rounds. The Kitchen quality drop therefore arises from the model's failure to *actively utilise* injected information during complex reasoning, rather than from information *loss* in the KV cache. This distinction motivates different mitigation strategies: RetroSIG addresses the utilisation gap, while CompSIG addresses cache size growth.
...
**Empirical findings.** On the EdgeAgent-Kitchen benchmark (35 steps, 4B GPU), SIG achieves 3.85x end-to-end speedup (3.25x under 8x FA projection), with AppLoop prefill at 36.1% of wall-clock vs SIG at 2.0%. KV-cache continuity improves information coverage from 4% to 33% in long-context scenarios. On 0.8B GPU, SIG's naive 1.11x speedup inverts at 2x FA (0.96x) due to generation inflation—confirming the >=1B applicability boundary and the necessity of CompSIG for sub-1B models. Cross-architecture validation on Gemma-4-E2B (SWA + shared_kv_layers + GQA-4) confirms 3.20× SIG speedup and perfect KV-cache factual recall (1.00 at 32 rounds), demonstrating architecture-agnostic applicability. Deep validation of KV-cache degradation across 64 injection rounds (13.6K tokens) shows no observable factual recall degradation, clarifying that the Kitchen benchmark's quality drop (0.92→0.52) reflects task-level information utilisation rather than cache-level information loss.
